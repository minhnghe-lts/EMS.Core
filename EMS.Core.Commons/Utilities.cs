...
namespace EMS.Core.Commons
{
    public static class Utilities
    {
        ...
        public static void PasswordValid(this string password)
        {
            ...
            if (!regex.IsMatch(password)) throw new InvalidFormatException(nameof(password));
        }
        public static void PhoneNumberValid(this string phone)
        {
            var pattern = @"^0[0-9]{9}$";
            var regex = new Regex(pattern);
            if (!regex.IsMatch(phone)) throw new InvalidFormatException(nameof(phone));
        }
        public static void EmailValid(this string email)
        {
            var pattern = @"^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$";
            var regex = new Regex(pattern);
            if (!regex.IsMatch(email)) throw new InvalidFormatException(nameof(email));
        }
        ...
        public static void IsDefineEnum(this Type type, Enum name)
        {
            ...
            if (string.IsNullOrEmpty(enumName)) throw new ItemNotFoundException();
        }
